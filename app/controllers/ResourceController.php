	...
	public function downloadResource($resourceId)
	{
		...
		if($resource->userid!=Auth::user()->id)
		{
            ...
            if (Auth::user()->profile->ifc > $ifc)
            {
                ...
                AjaxController::insertToNotification($resource->getAuthor->id,Auth::user()->id,"purchased","purchased your resource ".$resource->title,'http://localhost/b2v2/resource/'.$resource->id);

                TransactionController::insertToManager(Auth::user()->id,"-".$ifc,"Purchased resource",'http://localhost/b2v2/resource/'.$resource->id,$resource->title,"content");
                TransactionController::insertToManager($resource->getAuthor->id,"+".$ifc,"Resource purchased by",'http://localhost/b2v2/user/'.Auth::user()->username,Auth::user()->first_name.' '.Auth::user()->last_name,"profile");
            }
            else
